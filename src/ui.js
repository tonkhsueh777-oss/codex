(function (root) {
  ...
  function cardRuleText(card) {
    ...
    if (card.key === 'bully') return '指定一名其他玩家，使其下一完整回合无法行动。';
    if (card.key === 'fire') return '随机烧毁一名其他玩家的1张手牌；其下一回合开始不自动补牌至3张。';
    if (card.key === 'flower') return '指定一名其他玩家，与其直接交换当前位置。';
    ...
    refs.humanPanel.innerHTML = `
      ...
      <div class="human-actions">
        ...
        <div class="action-buttons">
          ...
          <button type="button" class="action-button action-button--primary" data-action="end" aria-label="换一张手牌并结束回合" ${!isHumanTurn || state.phase !== 'action' ? 'disabled' : ''}><span class="action-button__main">换一张手牌</span><span class="action-button__sub">结束回合</span></button>
        </div>
      </div>`;
  ...
  function showRules() {
    if (game.GuideOverlay) return game.GuideOverlay.open('manual');
    showChoiceDialog({
      title: '线上版核心规则',
      message: '每回合只能执行1个动作。恶霸王豹：指定对手下一完整回合无法行动。火烧百顺楼：随机烧毁对手1张手牌，其下一回合开始不自动补牌至3张。假绿菊花：与指定对手直接交换位置。嘉庆令、王德禄令：选择自己1件圣物，强制交换对手1件圣物，交换后立即检查所有玩家，集齐4种不同圣物立即获胜。地牌开放地点，巡游沿线移动1格，明察取得当地圣物。动作结算后补牌并结束回合；也可选择1张手牌换牌后结束。',
      choices: [],
      cancelText: '知道了'
    });
  }
